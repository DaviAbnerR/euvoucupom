...
$(document).ready(function() {
    ...
    const idCartela = params.get('cartela');
    const numeroCartela = params.get('numero');
    if (idCartela && numeroCartela) {
        abrirNumeros(idCartela, numeroCartela);
    }
});
...
function abrirNumeros(idCartela, numeroCartela) {
    ...
    $.getJSON(`/api/cartela/${idCartela}`, function(fichas) {
        ...
        // Adiciona o botão "Copiar Link" na modal-footer
        $('#modalNumeros .modal-footer').html(`
            <button class="btn btn-primary" id="copiarLinkCartela" data-id="${idCartela}" data-numero="${numeroCartela}">
                Copiar Link para esta Cartela
            </button>
        `);
...
$(document).on('click', '#copiarLinkCartela', function() {
    let idCartela = $(this).data('id');
    let numeroCartela = $(this).data('numero');
    // Pegue a URL atual da página (sem query) e adicione o parâmetro
    let baseUrl = window.location.origin + window.location.pathname;
    let link = `${baseUrl}?cartela=${idCartela}&numero=${numeroCartela}`;
    navigator.clipboard.writeText(link).then(() => {
        // Opcional: use seu toast ou alert
        alert('Link copiado!');
    });
});
